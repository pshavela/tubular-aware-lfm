# Adapted from https://github.com/nkicsl/Resfusion/blob/main/callback/ema.py
# with the following license: https://github.com/nkicsl/Resfusion/blob/main/LICENSE
#
# -------------------------------------------------------------------------------------------------------------------------------------
...
class EMA(Callback):
    ...
    def on_train_start(self, trainer: L.Trainer, l_module: L.LightningModule) -> None:
        ...
        if self._ema_model_weights is None:
            self._ema_model_weights = [p.detach().clone() for p in l_module.state_dict().values()]
        # ensure that all the weights are on the correct device
        self._ema_model_weights = [p.to(l_module.device) for p in self._ema_model_weights]
        self._overflow_buf = torch.IntTensor([0]).to(l_module.device)

    def ema(self, l_module: L.LightningModule) -> None:
        if apex_available and l_module.device.type == "cuda":
            return self.apply_multi_tensor_ema(l_module)
        return self.apply_ema(l_module)

    def apply_multi_tensor_ema(self, l_module: L.LightningModule) -> None:
        model_weights = list(l_module.state_dict().values())
    ...
    def apply_ema(self, l_module: L.LightningModule) -> None:
        for orig_weight, ema_weight in zip(list(l_module.state_dict().values()),
    ...
    def on_train_batch_end(
            self, trainer: L.Trainer, l_module: L.LightningModule, outputs: STEP_OUTPUT, batch: Any,
            batch_idx: int
    ) -> None:
    ...
    def on_load_checkpoint(
            self, trainer: L.Trainer, l_module: L.LightningModule, checkpoint: Dict[str, Any]
    ) -> None:
    ...
    def replace_model_weights(self, l_module: L.LightningModule) -> None:
        self._weights_buffer = [p.detach().clone().to("cpu") for p in l_module.state_dict().values()]
        new_state_dict = {k: v for k, v in zip(l_module.state_dict().keys(), self._ema_model_weights)}
        l_module.load_state_dict(new_state_dict)

    def restore_original_weights(self, l_module: L.LightningModule) -> None:
        state_dict = l_module.state_dict()
    ...
    def on_validation_start(self, trainer: L.Trainer, l_module: L.LightningModule) -> None:
        if self.ema_initialized and self.evaluate_ema_weights_instead:
            self.replace_model_weights(l_module)

    def on_validation_end(self, trainer: L.Trainer, l_module: L.LightningModule) -> None:
        if self.ema_initialized and self.evaluate_ema_weights_instead:
            self.restore_original_weights(l_module)

    def on_test_start(self, trainer: L.Trainer, l_module: L.LightningModule) -> None:
        if self.ema_initialized and self.evaluate_ema_weights_instead:
            self.replace_model_weights(l_module)

    def on_test_end(self, trainer: L.Trainer, l_module: L.LightningModule) -> None:
        if self.ema_initialized and self.evaluate_ema_weights_instead:
            self.restore_original_weights(l_module)
...
class EMAModelCheckpoint(ModelCheckpoint):
    ...
    def _update_best_and_save(
            self, current: Tensor, trainer: L.Trainer, monitor_candidates: Dict[str, Tensor]
    ) -> None:
