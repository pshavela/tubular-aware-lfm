# Adapted from https://github.com/Project-MONAI/GenerativeModels/blob/main/generative/networks/nets/vqvae.py
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        blocks.append(
            Convolution(
                spatial_dims=self.spatial_dims,
                in_channels=self.in_channels + self.concat_mask,
        ...
        for i in range(len(self.num_channels)):
            ...
            blocks.append(
                Convolution(
                    spatial_dims=self.spatial_dims,
                    in_channels=reversed_num_channels[i] + self.concat_mask,
    ...
    def forward(self, x: torch.Tensor, concat: torch.Tensor = None) -> torch.Tensor:
        for block in self.blocks:
            if isinstance(block, Convolution) and concat is not None:
                # downsample to latent size
                m = F.interpolate(concat, x.shape[2:], mode='nearest')
...
class VQVAE(nn.Module):
    ...
    def decode(self, quantizations: torch.Tensor, concat: torch.Tensor = None) -> torch.Tensor:
        if self.use_checkpointing:
            return torch.utils.checkpoint.checkpoint(self.decoder, quantizations, concat, use_reentrant=False)
        else:
            return self.decoder(quantizations, concat)
    ...
    def decode_stage_2_outputs(self, z: torch.Tensor, concat: torch.Tensor = None) -> torch.Tensor:
        e, _ = self.quantize(z)
        image = self.decode(e, concat)
        return image
