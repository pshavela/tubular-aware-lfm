# Adapted from https://github.com/Project-MONAI/MONAI/tree/dev/monai/networks/blocks/crossattention.py
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor | None = None, attn_mask: torch.Tensor | None = None):
        """
        Args:
            x (torch.Tensor): input tensor. B x (s_dim_1 * ... * s_dim_n) x C
            context (torch.Tensor, optional): context tensor. B x (s_dim_1 * ... * s_dim_n) x C
            attn_mask (torch.Tensor, optional): can contain positional bias values.
        ...
        if self.use_flash_attention:
            ...
            att_mat: torch.Tensor = torch.einsum("blxd,blyd->blxy", q, k) * self.scale
